...
class DataModel:
    def __init__(self, data, preproc_method=None):
        if preproc_method is not None:
            ...
            data['price_type'] = price_types.loc[list(data.index)]
            data['per_square_meter_price'] = target.loc[list(data.index)]
...
class HouseModel:
    ...
    def fit(self, data_model: DataModel):
        ...
        print(
            f'метрика предсказании пользовательских оценок {deviation_metric(data_model.y_test.values, default_prediction)}')
    ...
    def __create_model(self):
        ...
        model.add([
            ...
            RandomForestRegressor(n_estimators=300, random_state=21)
        ])

        model.add_meta(catboost.CatBoostRegressor(iterations=10000, random_state=21))
        return model
